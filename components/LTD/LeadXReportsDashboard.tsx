  ...
  return (
    <div className="w-full space-y-8 p-0 px-4 pb-12">
      ...
      <Dialog open={!!sessionToDelete} onOpenChange={() => setSessionToDelete(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-rose-600"><Trash2 className="h-5 w-5" /> Confirm Deletion</DialogTitle>
            <DialogDescription className="pt-2 text-sm text-zinc-600">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button variant="destructive" onClick={deleteSession} disabled={isDeleting}>{isDeleting ? 'Deleting...' : 'Delete'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
    ...
    <CardContent className="p-5">
      ...
      {isRate && value !== '—' && (
        <div className="w-full h-1 bg-zinc-100 mt-2 rounded-full overflow-hidden">
          <div className={`h-full transition-all duration-500 ${parseFloat(value) >= 80 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: value }} />
        </div>
      )}
    </CardContent>
  </Card>
...
const ChartCard = ({ title, data, color1, color2, placeholder }: any) => (
  <Card className="shadow-sm border-zinc-100">
    <CardHeader className="bg-zinc-50/50 border-b p-6">
      <CardTitle className="text-sm font-bold uppercase text-zinc-700 tracking-wider">{title}</CardTitle>
    </CardHeader>
    <CardContent className="p-6">
      <div className="h-[300px] w-full">
        {data.length === 0 ? (
          <div className="flex flex-col h-full items-center justify-center text-center p-6 bg-zinc-50/30 rounded-2xl border-2 border-dashed border-zinc-100">
            <Zap className="h-8 w-8 text-zinc-200 mb-3" />
            <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest leading-relaxed max-w-[180px]">
              {placeholder}
            </p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 20, right: 10, left: -20, bottom: 20 }}>
              ...
              <Bar dataKey="reg" name="Registrants" fill={color1} radius={[6, 6, 0, 0]} barSize={40}>
                <LabelList dataKey="reg" position="top" style={{ fontSize: 10, fontWeight: 'black', fill: color1 }} />
              </Bar>
              <Bar dataKey="att" name="Attendees" fill={color2} radius={[6, 6, 0, 0]} barSize={40}>
                <LabelList dataKey="att" position="top" style={{ fontSize: 10, fontWeight: 'black', fill: color2 }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        )}
