...
const ExecutiveDashboard: React.FC<ExecutiveDashboardProps> = ({ refreshTrigger }) => {
  ...
  const fetchBaseData = useCallback(async () => {
    ...
    try {
      ...
      // B. Fetch Cohort Metrics (Aggregated from Pre-assessment)
  ...
  useEffect(() => {
    if (selectedParticipant) {
      fetchIndividualData(selectedParticipant.id);
    }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-white dark:bg-zinc-900 p-4 rounded-xl border shadow-sm">
          <div className="flex flex-1 flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            {selectedParticipant && (
              <div className="w-full sm:w-auto">
                <label className="text-[10px] font-bold uppercase text-muted-foreground mb-1 block">Phase</label>
          ...
          {selectedParticipant && (
            <div className="flex items-center gap-3 border-l pl-4">
              <Building2 className="h-5 w-5 text-[#0046ab]" />
              <div>
          ...
          {!cohortMetrics ? (
            <Card className="border-dashed bg-muted/30 py-20 text-center">
              <CardContent>
                ...
                <h3 className="text-xl font-semibold">No Cohort Data Available</h3>
                <p className="text-muted-foreground">Upload pre-assessment data to see aggregate insights.</p>
              </CardContent>
            </Card>
          ) : (
            <>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <Card className="border-t-4 border-t-[#0046ab]">
                  <CardHeader className="pb-2">
                    <CardDescription className="text-[10px] font-bold uppercase tracking-wider text-[#0046ab]">Cohort Readiness Score</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="text-6xl font-black text-[#0046ab]">{cohortMetrics.readinessScore.toFixed(2)}</div>
                    <p className="text-xs text-muted-foreground mt-2">Baseline average across {cohortMetrics.totalParticipants} participants.</p>
                  </CardContent>
                </Card>

                <Card>
                  <CardHeader className="pb-2 text-green-600">
                    <CardTitle className="text-sm font-bold uppercase flex items-center gap-2"><TrendingUp className="h-4 w-4" /> Top Strengths</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    {cohortMetrics.topStrengths.map(s => (
                      <div key={s.competency} className="flex justify-between items-center text-sm font-medium">
                ...
                <Card>
                  <CardHeader className="pb-2 text-destructive">
                    <CardTitle className="text-sm font-bold uppercase flex items-center gap-2"><Target className="h-4 w-4" /> Dev Areas</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    {cohortMetrics.developmentAreas.map(s => (
                      <div key={s.competency} className="flex justify-between items-center text-sm font-medium">
                        <span>{s.competency}</span>
                        <Badge variant="secondary">{s.average}</Badge>
                      </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Cohort Competency Averages</CardTitle>
                  <CardDescription>Baseline performance across all departments.</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="h-[350px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={cohortMetrics.competencyAverages}>
                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
                        <XAxis dataKey="competency" axisLine={false} tickLine={false} tick={{ fontSize: 12 }} />
                        ...
                        <Bar dataKey="average" fill="#0046ab" radius={[4, 4, 0, 0]} barSize={50} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
              ...
              {/* Maturity Distribution and Dept Profile */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  <CardHeader>
                    <CardTitle>Maturity Distribution (%)</CardTitle>
                    <CardDescription>Breakdown of rating frequency per competency.</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="h-[350px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={cohortMetrics.distributionData}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} />
                          <XAxis dataKey="competency" tick={{ fontSize: 11 }} />
                          <YAxis unit="%" />
                          <Tooltip formatter={(v) => `${v}%`} />
                          <Legend />
                          <Bar dataKey="1" stackId="a" fill="#ef4444" name="Not Obs." />
                ...
                <Card>
                  <CardHeader>
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <CardDescription>Comparison across delivery units.</CardDescription>
                      </div>
                      <Select value={selectedCohortCompetency} onValueChange={setSelectedCohortCompetency}>
                  ...
                  <CardContent>
                    <div className="h-[350px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={cohortMetrics.departmentAverages} margin={{ bottom: 40 }}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} />
                          <XAxis dataKey="department" angle={-45} textAnchor="end" interval={0} tick={{ fontSize: 10 }} />
                          <YAxis domain={[0, 5]} />
                          <Tooltip />
                          <Bar dataKey={selectedCohortCompetency} fill="#0046ab" radius={[4, 4, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </CardContent>
      ...
      {dashboardType === 'individual' && (
        <div id="pdf-content-executive" className="animate-in fade-in duration-500">
          {!selectedParticipant ? (
            <Card className="border-dashed bg-muted/30 py-20 text-center">
              <CardContent>
                ...
                <h3 className="text-xl font-semibold">Select a participant to view their individual dashboard</h3>
                <p className="text-muted-foreground">Detailed competency growth and comparison data will appear here.</p>
              </CardContent>
            </Card>
          ) : isRefreshing ? (
            <div className="flex h-[400px] items-center justify-center">
          ...
          ) : !hasIndividualData(viewMode) ? (
            <Card className="border-dashed bg-muted/30 py-20 text-center">
              <CardContent>
                <Info className="mx-auto h-12 w-12 text-muted-foreground/40 mb-4" />
                <h3 className="text-xl font-semibold">No {viewMode.toUpperCase()} Data Found</h3>
                <p className="text-muted-foreground">Upload the relevant assessment file for this participant.</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <Card className="border-t-4 border-t-[#0046ab]">
                  <CardHeader className="pb-2">
                    <CardDescription className="text-[10px] font-bold uppercase tracking-wider text-[#0046ab]">
                      {viewMode === 'compare' ? 'Growth Readiness Score' : `${viewMode.toUpperCase()} Readiness Score`}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex items-baseline gap-3">
                      {(viewMode === 'pre' || viewMode === 'compare') && (
                        <div className="text-5xl font-black text-[#0046ab]">{processedIndividual.readinessScore.toFixed(2)}</div>
                      )}
                      {viewMode === 'compare' && <div className="text-2xl text-muted-foreground">→</div>}
                      {(viewMode === 'post' || viewMode === 'compare') && (
                        <div className="text-5xl font-black text-green-600">{processedIndividual.postReadinessScore.toFixed(2)}</div>
                      )}
                    </div>
                    {viewMode === 'compare' && (
                      <p className="text-xs font-bold text-green-600 mt-2">
                        Improvement: +{(processedIndividual.postReadinessScore - processedIndividual.readinessScore).toFixed(2)}
                      </p>
                    )}
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    {processedIndividual.topStrengths.map(s => (
                      <div key={s.competency} className="flex justify-between items-center text-sm font-medium">
                        <span>{s.competency}</span>
                        <Badge variant="secondary">{viewMode === 'post' ? s.post : s.pre}</Badge>
                      </div>
                    ))}
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    {processedIndividual.developmentAreas.map(s => (
                      <div key={s.competency} className="flex justify-between items-center text-sm font-medium">
                        <span>{s.competency}</span>
                        <Badge variant="secondary">{viewMode === 'post' ? s.post : s.pre}</Badge>
                      </div>
              ...
              {/* Main Individual Chart */}
              <Card>
                <CardHeader>
                  <CardTitle>Competency {viewMode === 'compare' ? 'Growth' : 'Profile'}</CardTitle>
                  <CardDescription>Individual assessment scores for {selectedParticipant.name}.</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="h-[400px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={processedIndividual.chartData}>
                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
                        <XAxis dataKey="competency" axisLine={false} tickLine={false} />
                        <YAxis domain={[0, 5]} axisLine={false} tickLine={false} />
                        <Tooltip cursor={{ fill: '#f3f4f6' }} />
                        <Legend verticalAlign="top" align="right" height={36} />
                        {(viewMode === 'pre' || viewMode === 'compare') && (
                          <Bar name="Pre" dataKey="pre" fill="#0046ab" radius={[4, 4, 0, 0]} barSize={viewMode === 'compare' ? 30 : 50} />
                        )}
                        {(viewMode === 'post' || viewMode === 'compare') && (
                          <Bar name="Post" dataKey="post" fill="#22c55e" radius={[4, 4, 0, 0]} barSize={viewMode === 'compare' ? 30 : 50} />
                        )}
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-5 gap-4">
                      {processedIndividual.chartData.map(d => (
                        <div key={d.competency} className="p-4 rounded-lg bg-zinc-50 dark:bg-zinc-800/50 border flex flex-col items-center gap-2">
                          <span className="text-[10px] font-bold uppercase text-muted-foreground">{d.competency}</span>
                          <div className="flex items-center gap-2">
                            <span className="text-lg font-bold">{d.delta > 0 ? `+${d.delta}` : d.delta}</span>
                            {d.delta > 0 ? <ArrowUpRight className="h-4 w-4 text-green-600" /> : d.delta < 0 ? <ArrowDownRight className="h-4 w-4 text-destructive" /> : <Minus className="h-4 w-4 text-muted-foreground" />}
                          </div>
                        </div>
                      ))}
                    </div>
