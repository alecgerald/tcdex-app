...
const PostLearningDashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (error) throw error;
      setRecords(data || []);
      setSelectedModule("All Modules"); // Reset module filter on session change
    } catch (err) {
  ...
  const handleDeleteSession = async (year: number, cohort: string) => {
    try {
  ...
  const handleExportPDF = useCallback(async () => {
    if (!metrics || !selectedSession) return;
    setIsExporting(true);
    const toastId = toast.loading("Generating vector PDF report...");

    const payload = {
      title: "Post-Learning Survey Dashboard",
      description: `Analysis for ${selectedSession.cohort} (${selectedSession.year}).`,
    ...
    try {
      ...
      a.href = url;
      a.download = `Post_Learning_Analysis_${selectedSession.cohort.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  }, [metrics, records, selectedSession]);
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-12 px-4">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-black uppercase tracking-widest text-zinc-500">Select Program Session</h2>
          </div>
          <Button 
        ...
        <div className="flex flex-nowrap overflow-x-auto gap-4 pb-4 -mx-1 px-1 scrollbar-hide">
          {sessions.map((session) => {
            ...
            return (
              <div key={`${session.year}-${session.cohort}`} className="relative group">
                <button
                  onClick={() => setSelectedSession(session)}
                  className={cn(
                    "flex-shrink-0 min-w-[180px] p-6 rounded-2xl border transition-all text-left",
                    isActive 
                ...
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <button
                      className={cn(
                        "absolute top-4 right-4 p-2 rounded-lg transition-colors cursor-pointer opacity-0 group-hover:opacity-100",
                        isActive 
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle className="font-black uppercase tracking-tight">Delete Session Data?</AlertDialogTitle>
                      <AlertDialogDescription className="text-zinc-500 font-medium">
                        This will permanently delete all survey records for <span className="font-bold text-zinc-800">{session.cohort} ({session.year})</span>. This action cannot be undone.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel className="font-bold uppercase text-xs rounded-xl">Cancel</AlertDialogCancel>
                      <AlertDialogAction 
                        onClick={() => handleDeleteSession(session.year, session.cohort)}
                        className="bg-rose-600 hover:bg-rose-700 text-white font-bold uppercase text-xs rounded-xl"
                      >
                        Delete Session
                      </AlertDialogAction>
                    </AlertDialogFooter>
          ...
          {sessions.length === 0 && (
            <div className="w-full py-8 text-center border-2 border-dashed border-zinc-100 rounded-3xl bg-zinc-50/50">
               <Zap className="h-8 w-8 text-zinc-300 mx-auto mb-3" />
               <p className="text-xs font-black uppercase text-zinc-400">No sessions available. Please upload survey data.</p>
            </div>
          )}
        </div>
      </div>
      ...
      {selectedSession && records.length > 0 ? (
        <div id="pdf-content-postlearning" className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
          {/* 1. OVERVIEW METRICS */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <Card className="shadow-sm border-zinc-100 rounded-3xl overflow-hidden">
              <CardHeader className="p-8 pb-4">
                ...
                <CardDescription className="text-xs font-bold uppercase text-zinc-400">Average effectiveness rating per module</CardDescription>
              </CardHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div>
                  ...
                  <div className="space-y-6">
                    ...
                    )) : <p className="text-base text-muted-foreground italic font-medium leading-relaxed text-center py-8 bg-zinc-50 rounded-2xl border border-dashed">No modules achieved the top performance threshold.</p>}
                  </div>
                </div>
        ...
        <Card className="border-dashed bg-zinc-50/50 py-32 text-center rounded-3xl">
          <CardContent>
            ...
            <h3 className="text-2xl font-black uppercase tracking-tight text-zinc-400">No Survey Data Available</h3>
            <p className="text-sm font-bold text-zinc-400 uppercase mt-3">Please upload a file to begin analysis</p>
          </CardContent>
        </Card>
      )}
    </div>
