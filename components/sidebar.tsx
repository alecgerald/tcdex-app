...
export function Sidebar({ onToggle }: SidebarProps) {
  ...
  // Determine which categories should be open based on the current path
  const initialOpenCategories = sidebarCategories
    .filter(cat => cat.items.some(item => pathname.startsWith(item.href)))
    .map(cat => cat.id)
  
  const [openCategories, setOpenCategories] = useState<string[]>(
    initialOpenCategories.length > 0 ? initialOpenCategories : ["lms"]
  )
  const username = "tcdexlms" // Hardcoded for this role
