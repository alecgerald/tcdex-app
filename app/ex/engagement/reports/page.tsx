...
export default function EngRetReportsPage() {
  ...
  const rehydrateIts = async () => {
    ...
    const mapped: ItsRawRow[] = data.map(r => ({
      "survey period":     (r.survey_period      ?? "").trim(),
      "function":          (r.function           ?? "").trim(),
      "role level":        (r.role_level         ?? "").trim(),
      "location":          (r.location           ?? "").trim(),
      "tenure band":       (r.tenure_band        ?? "").trim(),
      "q1 intent to stay": (r.q1_intent_to_stay  ?? "").trim(),
      "response date":     (r.response_date      ?? "").trim(),
    }))
    setItsRows(mapped)
  ...
  const nerParticipKey  = useMemo(() => nerKeys.find(k => k.includes("at least one") || k.includes("employees who made")) ?? "", [nerKeys])
  ...
  const itsFig2 = useMemo(() => { const map: Record<string, { fav: number; total: number }> = {}; itsFiltered.forEach(r => { console.log("tenure band raw:", JSON.stringify(r["tenure band"]), "itsQ1Key:", itsQ1Key, "q1 value:", JSON.stringify(r[itsQ1Key]), "isAgree:", isAgree(r[itsQ1Key])); const band = (r["tenure band"] as string | undefined)?.trim() ?? "Unknown"; if (!map[band]) map[band] = { fav: 0, total: 0 }; map[band].total++; if (isAgree(r[itsQ1Key])) map[band].fav++ }); return Object.entries(map).map(([band, d]) => ({ band, rate: pct(d.fav, d.total) })) }, [itsFiltered, itsQ1Key])
  ...
  const itsC        = (r: number) => r >= 80 ? "#8b5cf6" : r >= 60 ? "#8b5cf6" : "#8b5cf6"
